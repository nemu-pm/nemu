...
import { useDrawerPortalContainer } from "./drawer"
...
function DropdownMenuContent({
  ...
  >) {
  // When inside a drawer, render portal inside the drawer to work with modal behavior
  const drawerContainer = useDrawerPortalContainer()
  
  // Only pass container prop when we have an actual container
  // undefined vs not-passed behaves differently in Base UI
  const portalProps = drawerContainer ? { container: drawerContainer } : {}
    ...
    <MenuPrimitive.Portal {...portalProps}>
      ...
      >
        <MenuPrimitive.Popup
          ...
          className={cn("data-open:animate-in data-closed:animate-out data-closed:fade-out-0 data-open:fade-in-0 data-closed:zoom-out-95 data-open:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 ring-foreground/10 bg-popover text-popover-foreground min-w-32 rounded-md p-1 shadow-md ring-1 duration-100 z-[100] max-h-(--available-height) w-(--anchor-width) origin-(--transform-origin) overflow-x-hidden overflow-y-auto outline-none data-closed:overflow-hidden", className )}
          {...props}
        />
      </MenuPrimitive.Positioner>
