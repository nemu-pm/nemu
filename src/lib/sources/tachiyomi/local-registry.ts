...
} from "@nemu.pm/tachiyomi-runtime/async";
import {
  createTachiyomiBrowsableSource,
...
export class TachiyomiLocalRegistry implements SourceRegistryProvider {
  ...
  private async ensureInitialized(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `[TachiyomiLocal] Found ${extensionPaths.length} local extensions`,
      );

      for (const extPath of extensionPaths) {
        try {
          const manifestRes = await fetch(
            `/local-extensions/${extPath}/manifest.json`,
          );
          ...
          const iconUrl = manifest.icon
            ? `/local-extensions/${extPath}/${manifest.icon}`
            : undefined;

          // Store extension info (sources loaded lazily via getSource)
          this.extensions.set(extPath, {
            path: extPath,
            manifest,
            jsUrl,
            iconUrl,
          ...
          console.log("[TachiyomiLocal] Loaded a local extension");
        } catch (e) {
          console.warn(
            "[TachiyomiLocal] Failed to load a local extension:",
            safeErrorCategory(e),
          );
        }
      }
      ...
      console.log(
        "[TachiyomiLocal] Could not fetch index.json:",
        safeErrorCategory(e),
      );
    }
  ...
  async getSource(sourceId: string): Promise<MangaSource | null> {
    ...
    const ext = this.extensions.get(sourceId);
    if (!ext) {
      console.warn("[TachiyomiLocal] Extension not found");
      return null;
    }
    ...
    try {
      ...
      const savedSettings =
        settingsStore.getState().values.get(sourceKey) ?? {};
      const savedSourceId = savedSettings[SOURCE_SELECTION_KEY] as
        | string
        | undefined;
      ...
      console.log("[TachiyomiLocal] Loaded a source");
      return source;
    } catch (e) {
      console.error(
        "[TachiyomiLocal] Failed to load a source:",
        safeErrorCategory(e),
      );
      return null;
    }
