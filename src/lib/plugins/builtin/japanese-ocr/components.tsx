...
export function DetectionOverlay({ pageIndex, ctx }: DetectionOverlayProps) {
  ...
  const blocks = detections.get(pageIndex) ?? []
  ...
  return (
    <Fragment>
      ...
      >
        ...
        <span
          className={cn(
            "relative inline-block rounded-[4px]",
          ...
          style={{
            background: isSelected
              ? 'color-mix(in oklch, var(--pos-bg) 70%, white 30%)'
              : 'var(--pos-bg)',
            borderBottom: '2px solid var(--pos-border)',
            boxShadow: isSelected
              ? '0 0 0 1.5px color-mix(in oklch, var(--pos-border) 60%, transparent), 0 4px 12px -2px color-mix(in oklch, var(--pos-border) 30%, transparent)'
              : undefined,
          }}
        >
          {displayWord}
  ...
  return (
    <div className={cn("space-y-2", posClass)}>
      ...
      <div className="flex items-baseline gap-3 flex-wrap">
        <span
          className="ja-textbook text-2xl sm:text-3xl font-semibold tracking-tight text-foreground"
          lang="ja"
        >
          {token.word}
        </span>
        {token.reading && (
          <span className="text-base text-muted-foreground font-normal" lang="ja">
            {token.reading}
          </span>
        )}
      </div>
          ...
          <div className="pt-2">
            ...
            <div className="flex flex-wrap gap-1.5 items-center mb-3">
              {token.components.map((component, i) => (
                <Fragment key={i}>
                  <span
                    className="inline-flex items-center px-2.5 py-1 rounded-lg bg-secondary/80 text-sm font-medium"
                    lang="ja"
                  >
                    {component.word}
                  </span>
