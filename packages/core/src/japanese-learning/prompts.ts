/**
 * Japanese-learning / Nemu Chat prompt builders, shared by web and mobile.
 *
 * Previously duplicated: web had these in
 * `src/lib/plugins/builtin/japanese-learning/chat/prompts.ts` and mobile had a
 * parallel copy in `apps/mobile/src/lib/mobileJapaneseLearningChat.ts`. They are
 * now canonical here so both apps send identical greeting / explain prompts
 * (mobile navbar chat action parity — T6.1).
 */

export type PromptLocale = 'en' | 'ja' | 'zh'
export type NemuResponseMode = 'app' | 'jlpt'

export type ExplainKind = 'sentence' | 'word' | 'words'
