...
const ANDROID_LINT_IGNORE_ATTRIBUTE = "tools:ignore";
const ANDROID_NOTIFICATION_PERMISSION_LINT_ID = "NotificationPermission";
const ANDROID_SCREEN_ORIENTATION_ATTRIBUTE = "android:screenOrientation";
const ANDROID_OVERLAY_PERMISSION =
  '<uses-permission android:name="android.permission.SYSTEM_ALERT_WINDOW"/>';
const ANDROID_REMOVED_OVERLAY_PERMISSION =
...
`;
const ANDROID_LINT_XML = `<?xml version="1.0" encoding="UTF-8"?>
<!--
  Generated by plugins/with-third-party-jsc.cjs during prebuild. Do not edit.

  expo-audio's dependency bytecode always contains AudioControlsService and
  its notification call, even when enableBackgroundPlayback=false removes the
  service from the merged manifest. Ignore only that unreachable library
  finding. Release-manifest tests separately require the service and every
  notification/background-audio permission to remain absent.
-->
<lint>
...
gradle.projectsEvaluated {
    ...
    // React Native Skia exposes Prefab packages to the app, but its Gradle
    // plugin does not declare the app CMake configure tasks as consumers of
    // those package tasks. On a fresh or combined clean build, app Prefab
    // configuration can consequently race ahead and fail because Skia's
    // build/intermediates/prefab_package directory does not exist yet.
    def skiaProject = rootProject.findProject(":shopify_react-native-skia")
    def skiaPrefabPackageTasks = skiaProject == null ? [] :
        skiaProject.tasks.findAll { task ->
            task.name == "prefabDebugPackage" ||
                task.name == "prefabReleasePackage"
        }
    tasks.findAll { task -> task.name.startsWith("configureCMake") }.each { appConfigure ->
        def releaseBuild =
            appConfigure.name.contains("Release") ||
                appConfigure.name.contains("RelWithDebInfo")
        def producerName = releaseBuild ?
            "prefabReleasePackage" : "prefabDebugPackage"
        def producer = skiaProject?.tasks?.findByName(producerName)
        if (producer != null) {
            appConfigure.dependsOn(producer)
        }
    }
    ...
    if (combinedCleanBuild) {
        ...
        skiaPrefabPackageTasks.each { prefabPackage ->
            allProjectCleanTasks.each { projectClean ->
                prefabPackage.mustRunAfter(projectClean)
            }
        }
    } else {
...
function removeLegacyAndroidBackgroundAudioLintIgnore(androidManifest) {
  const application = androidManifest.manifest.application?.[0];
  if (!application) {
    throw new Error("Android manifest is missing its application element.");
  }

  // Earlier prebuilds placed NotificationPermission on the whole application.
  // Expo can reuse generated manifests, so actively migrate that broad ignore
  // to app/lint.xml while preserving any unrelated annotations.
  const ignored = String(application.$?.[ANDROID_LINT_IGNORE_ATTRIBUTE] ?? "")
    .split(",")
    .map((item) => item.trim())
    .filter(
      (item) =>
        item.length > 0 && item !== ANDROID_NOTIFICATION_PERMISSION_LINT_ID,
    );
  if (ignored.length > 0) {
    application.$ = {
      ...application.$,
      [ANDROID_LINT_IGNORE_ATTRIBUTE]: ignored.join(","),
    };
  } else if (application.$) {
    delete application.$[ANDROID_LINT_IGNORE_ATTRIBUTE];
  }
  return androidManifest;
}

function removeRedundantAndroidDefaultOrientation(androidManifest) {
  const activities = androidManifest.manifest.application?.[0]?.activity ?? [];
  for (const activity of activities) {
    if (activity.$?.[ANDROID_SCREEN_ORIENTATION_ATTRIBUTE] === "unspecified") {
      delete activity.$[ANDROID_SCREEN_ORIENTATION_ATTRIBUTE];
    }
  }
  return androidManifest;
}
...
async function writeAndroidLintConfig(platformProjectRoot) {
  await fs.writeFile(
    path.join(platformProjectRoot, "app/lint.xml"),
    ANDROID_LINT_XML,
  );
}

function patchAndroidDebugManifest(contents) {
  if (contents.includes(ANDROID_REMOVED_OVERLAY_PERMISSION)) {
    return contents;
  }
  return replaceOnceOrThrow(
    contents,
    ANDROID_OVERLAY_PERMISSION,
    ANDROID_REMOVED_OVERLAY_PERMISSION,
    "remove debug overlay permission",
  );
}

async function removeAndroidDebugOverlayPermission(platformProjectRoot) {
  for (const buildType of ["debug", "debugOptimized"]) {
    const manifestPath = path.join(
      platformProjectRoot,
      `app/src/${buildType}/AndroidManifest.xml`,
    );
    let contents;
    try {
    ...
    await fs.writeFile(manifestPath, patchAndroidDebugManifest(contents));
  }
}
...
function withThirdPartyJsc(config) {
  ...
  config = withAndroidManifest(config, (manifestConfig) => {
    ...
    manifestConfig.modResults = removeLegacyAndroidBackgroundAudioLintIgnore(
      manifestConfig.modResults,
    );
    manifestConfig.modResults = removeRedundantAndroidDefaultOrientation(
      manifestConfig.modResults,
    );
  ...
  config = withFinalizedMod(config, [
    ...
    async (finalizedConfig) => {
      ...
      await writeAndroidLintConfig(
        finalizedConfig.modRequest.platformProjectRoot,
      );
      await removeAndroidDebugOverlayPermission(
        finalizedConfig.modRequest.platformProjectRoot,
      );
...
module.exports.removeLegacyAndroidBackgroundAudioLintIgnore =
  removeLegacyAndroidBackgroundAudioLintIgnore;
module.exports.removeRedundantAndroidDefaultOrientation =
